...
import {
  ...
  getTokenTemplates,
  getDisabledChannels,
  rebuildChannelsForChannelTemplate,
} from './api'
import type { ChannelTemplate, TokenTemplate, DisabledChannel } from './types'
...
export function ChannelTemplates() {
  ...
  const { data: tokenTemplatesData } = useQuery({
  ...
  const templates = (data?.data ?? []) as ChannelTemplate[]
  const tokenTemplates = (tokenTemplatesData?.data ?? []) as TokenTemplate[]
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof EMPTY_FORM) => createChannelTemplate(data),
    onSuccess: (res) => {
      if (res.success) {
        toast.success(t('Created successfully'))
        queryClient.invalidateQueries({ queryKey: ['channel-templates'] })
        closeForm()
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteChannelTemplate(id),
    onSuccess: (res) => {
      if (res.success) {
  ...
  const rebuildMutation = useMutation({
    mutationFn: (templateId: number) => rebuildChannelsForChannelTemplate(templateId),
    onSuccess: (res) => {
      if (res.success) {
  ...
  function getChannelTemplateName(id: number): string {
    const ch = disabledChannels.find((c) => c.id === id)
    return ch ? ch.name : '—'
  }

  function getTokenTemplateName(id: number): string {
    if (id === 0) return t('Self (use own login config)')
    const tmpl = tokenTemplates.find((tt) => tt.id === id)
    return tmpl ? tmpl.name : `#${id}`
  }
  ...
  return (
    ...
    <SectionPageLayout>
      ...
      <SectionPageLayout.Description>
        {t('Define channel blueprints that auto-create a channel for each user token.')}
      </SectionPageLayout.Description>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t('Template Channel')}</TableHead>
                <TableHead>{t('Token Template')}</TableHead>
            ...
            <TableBody>
              {templates.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{getChannelTemplateName(row.channel_template_id)}</TableCell>
                  <TableCell>{getTokenTemplateName(row.token_template_id)}</TableCell>
