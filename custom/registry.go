...
func RegisterMigrations(database *gorm.DB) {
	...
	database.AutoMigrate(&token_config.ChannelTemplate{})
...
func RegisterRoutes(selfRoute *gin.RouterGroup, adminRoute *gin.RouterGroup, channelRoute *gin.RouterGroup, rootRouter *gin.RouterGroup) {
	...
	// Admin-only: CRUD token templates
	...
	// Channel templates (admin-only CRUD, but also readable by users for selection)
	tcRoute.GET("/channel-templates", token_config.GetChannelTemplates)
	adminRoute.POST("/token-config/channel-templates", token_config.CreateChannelTemplate)
	adminRoute.PUT("/token-config/channel-templates/:id", token_config.UpdateChannelTemplate)
	adminRoute.DELETE("/token-config/channel-templates/:id", token_config.DeleteChannelTemplate)
	adminRoute.POST("/token-config/channel-templates/:id/rebuild-channels", token_config.RebuildChannelsForChannelTemplate)
